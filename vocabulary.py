...
from misc.vocab_logging import logged_vocab_method
...
class Vocab:
  ...
  @classmethod
  @logged_vocab_method('create_from_freq_dict')
  def create_from_freq_dict(cls, freq_dict: Dict[str, int]):
    ...
    if len(sorted_by_occurrences) > config.MAX_NUMBER_OF_WORDS_IN_FREQ_DICT:
      sorted_by_occurrences = \
        sorted_by_occurrences[:config.MAX_NUMBER_OF_WORDS_IN_FREQ_DICT]
    return cls(words=sorted_by_occurrences)

  @classmethod
  @logged_vocab_method('load_from_file')
  ...
  @logged_vocab_method('save_to_file')
  ...
  def get_word_to_index_lookup_table(self) -> tf.lookup.StaticHashTable:
    if self.lookup_table_word_to_index is None:
      self.lookup_table_word_to_index = self.create_word_to_index_lookup_table(
        self.word_to_index,
        config.DEFAULT_INT32_LOOKUP_VALUE)
  ...
  def get_index_to_word_lookup_table(self) -> tf.lookup.StaticHashTable:
    if self.lookup_table_index_to_word is None:
      self.lookup_table_index_to_word = self.create_index_to_word_lookup_table(
        self.index_to_word,
        config.DEFAULT_STRING_LOOKUP_VALUE)
...
class Code2VecVocabs:
  def __init__(self):
    ...
    if config.CREATE_VOCAB:
      ...
      self._load(config.CODE2VEC_VOCABS_PATH)

  @logged_vocab_method('_create')
  def _create(self):
